...
// İlk senkron (syncToken yokken) Google'ın takvimdeki HER ŞEYİ (yıllar
// süren geçmiş dahil) döndürmesine yol açar — Nurlana'nın takvimi yıllardır
// kullandığı bir klinik için bu, tek istekte binlerce etkinlik demek. Bu hem
// sunucusuz fonksiyon zaman aşımına uğrayıp syncToken'ın hiç kaydedilememesine
// (her tetiklemede en baştan sonsuz döngü) hem de CRM'in ilgisiz geçmişle
// dolmasına yol açıyordu. Nurlana'nın tercihi: geçmiş önemli değil, CRM sadece
// bundan sonraki randevular için — bu yüzden ilk senkronu "şu andan itibaren"
// ile sınırlıyoruz. syncToken bir kez kaydedildikten sonra sonraki tüm
// senkronlar zaten olay bazlı (Google'ın kendi delta mekanizması) çalışır,
// timeMin'e ihtiyaç duymaz.
...
export async function pullAndApplyChanges() {
  ...
  do {
    ...
    // Her sayfayı hemen işliyoruz (hepsini biriktirip sona bırakmak yerine) —
    // böylece fonksiyon zaman aşımına uğrarsa bile o ana kadar işlenen
    // randevular kalıcı olur, bir sonraki deneme sıfırdan başlamak zorunda
    // kalmaz.
